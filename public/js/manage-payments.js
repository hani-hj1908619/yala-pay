...
async function onChequeSelect() {
    ...
    payImageUrl.addEventListener('change', function () {
        ...
        if(this.files[0].size/1024 > 1500)
        {
            payImageUrl.parentElement.setAttribute('data-error', "Cheque Image size limit exceeded!. " +
                "Please attach an image file with a size of less than 1.5MB.")
    ...
    if (isNaN(paymentNo)) {
        ...
        if (paymentMode.value == "Cheque") {
            ...
            cheque = new Cheque(parseInt(payChequeId.value),(paymentAmount.value), payChequeDrawer.value, payBankSelect.value,
                payChequeStatus.value, payChequeIssueDate.value, payChequeDueDate.value, payImage.getAttribute("src"),lastPayment[0].paymentId)


            // empty fields error validation for cheque
            if (!await chequeErrorValidation(cheque, false)) {
            ...
            payment.paymentId = lastPayment[0].paymentId
            console.log(payment)
        ...
        if (isCheque == true) {     // updating cheque
            ...
            const cheque = await chequeRepo.getChequeByPaymentId(paymentId.value)
            ...
            //error validation
            if (!await chequeErrorValidation(cheque, true))
                return
...
async function chequeErrorValidation(cheque, updateFlag) {
    let valid = true;
    const chequeExists = await chequeRepo.getChequeByChequeNo(cheque.chequeNo);
    if (chequeExists && !updateFlag) {
        console.log("inside")
